...
var vm = new Vue({
  ...
  computed: {
    ...
    navTitle: function navTitle() {
      if (this.activePage === 1) return '畢業學分統計';else if (this.activePage === 2) return '畢業學分列表';else if (this.activePage === 3) return '課表';else if (this.activePage === 4) return '個人資料';else if (this.activePage === 5) return '項目列表';
    },
  ...
  methods: {
    getData: function getData(data) {
      ...
      _.each(this.creditSummary, function (iv, year) {
        ...
        _.each(_this2.creditSummary[year]['total'], function (jv) {
          var credit = _.parseInt(jv['畢業學分']);
          var score = _.parseInt(jv['成績']);
          credits += credit;
          if (score >= 60) {
            getCredits += credit;
          }
    ...
    getCourseInfo: function getCourseInfo(info) {
      var html = '<ul class="courseInfo"><li>學年：' + info['學年'] + '</li><li>學期：' + info['學期'] + '</li><li>選課號碼：' + info['選課號碼'] + '</li><li>課程名稱：' + info['課程名稱'] + '</li><li>開課系所：' + info['開課系所'] + '</li><li>畢業學分：' + info['畢業學分'] + '</li><li>成績：' + info['成績'] + '</li></ul>';
